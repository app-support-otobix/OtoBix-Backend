...
function applyOfferMarginsFromCar(schema, {
    carModelPath = '../Models/carModel',
    carIdField = 'carId',        // offer.carId (string ObjectId)
    fixedField = 'fixedMargin',
    variableField = 'variableMargin',
    carSelect = 'priceDiscovery fixedMargin variableMargin',
    alwaysRefreshOnUpdate = false, // set true if you want margins on every update
} = {}) {
    ...
    async function fillDocMargins(doc, car) {
    ...
    // ✅ optional: refresh margins on update (if you want)
    schema.pre('findOneAndUpdate', async function (next) {
        try {
            const opts = this.getOptions() || {};
            if (!alwaysRefreshOnUpdate && !opts.upsert) return next();
            ...
            // if upsert insert, also apply on insert
            if (opts.upsert) {
                update.$setOnInsert = update.$setOnInsert || {};
                // if (update.$setOnInsert[fixedField] == null) update.$setOnInsert[fixedField] = FIXED_MARGIN;
                if (update.$setOnInsert[fixedField] == null) update.$setOnInsert[fixedField] = fixedMargin;

                if (update.$setOnInsert[variableField] == null) update.$setOnInsert[variableField] = update.$set[variableField];
            }
...
module.exports = applyOfferMarginsFromCar;
