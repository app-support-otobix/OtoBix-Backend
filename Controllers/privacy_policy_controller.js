// controllers/privacy_policy_controller.js
...
async function nextVersion(db) {
    ...
    if (!doc || typeof doc.seq !== 'number') {
        ...
        const check = await counters.findOne({ _id: 'privacyPolicy' });
...
function deriveTitle({ providedTitle, fileName, html }) {
    ...
    return (fileName || 'Privacy Policy').replace(/\.[^.]+$/, '');
}
...
// POST /api/privacyPolicy/upload  (form-data: file=<docx/pdf>, title=<optional>)
exports.uploadPrivacyPolicy = async (req, res) => {
    try {
        if (!req.file?.buffer) {
        ...
        const col = db.collection('privacyPolicy');
        ...
        if (exists) {
            return res.status(200).json({
                success: true,
                message: "No changes found. The uploaded document is same as the current Privacy Policy.",
                data: {
        ...
        return res.status(201).json({
            success: true,
            message: 'New Privacy Policy version created.',
            data: {
                version: doc.version,
                title: doc.title,
                content: doc.content,
        ...
        console.error('[uploadPrivacyPolicy] Error:', err);
        res.status(500).json({ success: false, message: err.message || 'Upload failed' });
    }
};

// GET /api/privacyPolicy/latest
exports.getLatestPrivacyPolicy = async (req, res) => {
    try {
        ...
        console.error('[getLatestPrivacyPolicy] Error:', err);
        res.status(500).json({ success: false, message: 'Failed to fetch latest privacy policy' });
    }
};

// GET /api/privacyPolicy/:version
exports.getPrivacyPolicyByVersion = async (req, res) => {
    try {
        ...
        console.error('[getPrivacyPolicyByVersion] Error:', err);
        res.status(500).json({ success: false, message: 'Failed to fetch version' });
    }
};

// GET /api/privacyPolicy?limit=10&page=1
exports.listPrivacyPolicy = async (req, res) => {
    try {
        ...
        console.error('[listPrivacyPolicy] Error:', err);
        res.status(500).json({ success: false, message: 'Failed to list privacy policy' });
    }
};
