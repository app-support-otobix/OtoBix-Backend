// controllers/dealer_guide_controller.js
...
async function nextVersion(db) {
    ...
    if (!doc || typeof doc.seq !== 'number') {
        ...
        const check = await counters.findOne({ _id: 'dealerGuide' });
...
function deriveTitle({ providedTitle, fileName, html }) {
    ...
    return (fileName || 'Dealer Guide').replace(/\.[^.]+$/, '');
}
...
// POST /api/dealerGuide/upload  (form-data: file=<docx/pdf>, title=<optional>)
exports.uploadDealerGuide = async (req, res) => {
    try {
        if (!req.file?.buffer) {
        ...
        const col = db.collection('dealerGuide');
        ...
        if (exists) {
            return res.status(200).json({
                success: true,
                message: "No changes found. The uploaded document is same as the current Dealer Guide.",
                data: {
        ...
        return res.status(201).json({
            success: true,
            message: 'New Dealer Guide version created.',
            data: {
                version: doc.version,
                title: doc.title,
                content: doc.content,
        ...
        console.error('[uploadDealerGuide] Error:', err);
        res.status(500).json({ success: false, message: err.message || 'Upload failed' });
    }
};

// GET /api/dealerGuide/latest
exports.getLatestDealerGuide = async (req, res) => {
    try {
        const db = mongoose.connection;
        const col = db.collection('dealerGuide');
        const doc = await col.find().sort({ version: -1 }).limit(1).next();
        if (!doc) return res.status(404).json({ success: false, message: 'No dealer guide found' });
        ...
        console.error('[getLatestDealerGuide] Error:', err);
        res.status(500).json({ success: false, message: 'Failed to fetch latest dealer guide' });
    }
};

// GET /api/dealerGuide/:version
exports.getDealerGuideByVersion = async (req, res) => {
    try {
        ...
        console.error('[getDealerGuideByVersion] Error:', err);
        res.status(500).json({ success: false, message: 'Failed to fetch version' });
    }
};

// GET /api/dealerGuide?limit=10&page=1
exports.listDealerGuide = async (req, res) => {
    try {
        ...
        console.error('[listDealerGuide] Error:', err);
        res.status(500).json({ success: false, message: 'Failed to list dealer guide' });
    }
};
